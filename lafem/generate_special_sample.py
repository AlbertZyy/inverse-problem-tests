...
def infnorm(p: Tensor):
    return bm.max(bm.abs(p), axis=-1)
...
def main(sigma_iterable: Sequence[int], seed: int = 0, index: int = 0):
    ...
    for sigma_idx in tqdm(sigma_iterable,
                          ...
                          position=index):
        # choose from 3, 4, 5
        NUM_CIR = np.random.randint(3, 6)

        while True:
            ctrs_ = np.random.rand(NUM_CIR, 2) * 1.6 - 0.8 # (NCir, GD)
            infrad = infnorm(ctrs_) # (NCir, )

            if (np.sum(infrad > 0.6) >= 2) and (np.sum(infrad < 0.2) >= 1):
                break

        norm = np.max(np.abs(ctrs_), axis=-1)
        if np.sum(norm > 0.6) < 2:
            pass
