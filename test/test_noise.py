...
dataset = TPZDataset(r"./data/gdgn_64_64_train", 1000, tqdm=True)
gdgn = dataset.data
...
def sum_energy(x: Tensor) -> Tensor:
    ...
    x = x.reshape(BATCH, CHANNEL, -1, 2)
    return x.norm(dim=-1).mean(dim=0) # [CHANNEL, ]
...
print(f"Energy of data: {energy_data.item():.4e}")
print(f"Energy of noise: {energy_noise.item():.4e}")
print(f"Energy of total: {energy_total.item():.4e}")

print(f"NOISE/DATA: {100*energy_noise.item() / energy_data.item():.2f}%")
