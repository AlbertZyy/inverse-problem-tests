...
from cnn import build_model as build_model_cnn
...
denoiser, _  = build_model_cnn(device, tag='C')
denoiser.eval()
...
def train(epoch: int):
    ...
    for gdgn, label in tqdm(train_dataset.loader(data_conf['train_batch_size']),
        ...
        noise = gdgn[:, :, 0, :] * noise
        gdgn[:, :, 0, :] += noise
        gdgn = denoiser(gdgn.reshape(-1, 8*2, 252)).reshape(-1, 8, 2, 252)

        y_out = model(gdgn) # (N, 1, Nx, Ny)
