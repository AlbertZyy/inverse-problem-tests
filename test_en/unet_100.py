...
from fractional import EigenNorm
...
def build_model(device: device, tag: str, *args, **kwargs):
    EXT = 63
    H = 2./EXT

    lsolver = LaplaceFDMSolver([EXT, EXT], [H, H], device=device)
    frac = EigenNorm(252, 8, momentum=0.999, device=device)
    ...
    try:
        model.load_state_dict(torch.load(f"./test_en/checkpoints/{FULL_NAME}.pth", map_location=device))
