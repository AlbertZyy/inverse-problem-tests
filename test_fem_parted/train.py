...
def train(epoch: int, stage: int):
    ...
    s: Tensor = model.df_solver.bc_filter.s

    if stage == 0:
        snoise = torch.randn_like(s, requires_grad=False) * 0.17
        with torch.no_grad():
            s.add_(snoise)
...
for epoch in trange(0, n_epoch//3, desc='Stage 0', unit='epoch'):
    train(epoch, 0)
    validate(epoch)

for epoch in trange(n_epoch//3, n_epoch - n_epoch//3, desc='Stage 1', unit='epoch'):
    train(epoch, 1)
    validate(epoch)
