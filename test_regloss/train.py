...
from fractional import Fractional, RegressionLoss
...
lr              = config['lr']
regloss_coef: float    = config.get('regloss_coef', 0.0)
...
df_solver = model.df_solver
frac = df_solver._frac
...
print(f"RegLoss coef: {regloss_coef}")
...
reg_loss_fn = RegressionLoss(252, 8, weight=True, device=device)
reg_loss_fn.reset(frac.w)
...
def train(epoch: int):
    ...
    for gdgn, label in tqdm(train_dataset.loader(data_conf['train_batch_size']),
                            desc=f'Epoch {epoch + 1}/{n_epoch}, Loss: {loss.item():.4f}', unit='batch', leave=False):
        optim.zero_grad()
        ...
        loss += regloss_coef * reg_loss_fn(frac.s, frac.decompose(df_solver.frac_input())).to(dtype=torch.float32)
        loss.backward()
        optim.step()
        step += 1
...
def validate(epoch):
    ...
    with torch.no_grad():
        ...
        loss = loss_fn(y_out, label.flatten().to(dtype=torch.float32, device=device))
        loss += regloss_coef * reg_loss_fn(model.df_solver._frac.s, model.df_solver.frac_input()).to(dtype=torch.float32)
