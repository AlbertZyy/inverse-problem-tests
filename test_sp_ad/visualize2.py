...
validation_set = NPZDataset('D:\Data\gdgn_cir3_e64_64_c8_validate', names=-1,
                            channel_keys=['gd'])
gn = np.load('D:\Data\gdgn_cir3_e64_64_c8_validate\gn.npy')
gn = torch.from_numpy(gn).to(device)
...
# gss = {i: figs[i].add_gridspec(figure_matrix[0]+1, figure_matrix[1]+1) for i in ID}
...
for tag, type_, noise_coef, noise_filter, ckpts_path in settings:
    ...
    for i in ID:
        ...
        data = validation_set._read_data(f"gd_{i}")[0].clone().to(device) # (8, 252)

        noise = torch.randn_like(data[..., :]) * noise_coef
        if noise_filter:
            noise = noise_filter(noise)
        noise = data[..., :] * noise
        data[..., :] += noise
        data = torch.stack([data, gn], dim=1) # (8, 2, 252)
        ...
        file_ = np.load(f'D:\Data\gdgn_cir3_e64_64_c8_validate\gd_{i}.npz')
...
if not NO_PLOT:
    for i in ID:
        ...
        figs[i].savefig(f'{save_dir}vis_cir3_{i}_new.png')
